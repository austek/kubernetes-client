...
|---|---|---|---|
| **Modules covered without other changes** | **59 of 74** — every `packaging=bundle` module gets its main jar's descriptor for free | **1 of 7** slice modules as measured — confounded, see Calibration. The one module that succeeded (`zjsonpatch`) is `packaging=bundle`, so **moditect's claimed "writes the main jar regardless of packaging" advantage is NOT empirically confirmed**: all three `packaging=jar` slice modules failed, for the dependency-naming reason, before `add-module-info` ever succeeded. Structurally plausible from the wiring (`overwriteExistingFiles` targets `project.build.finalName`'s own jar), unverified here. Repo-wide coverage also **not measured** (would need Task 5's config applied to all 74 with an ordered `install`) | `11-bnd:229-230`, `11-bnd:254-260`; `20-mod:432`, `20-mod:319-329`, `20-mod:642-657` |
| **Prerequisite change needed** | Convert the **9** wired `packaging=jar` modules to `packaging=bundle`. Prototype proven on `kubernetes-client-api`: main jar gained the real descriptor and the `-bundle` classified artifact disappeared. Cost: drops a published classified artifact; `platforms/karaf/features/src/main/resources/feature.xml:65,66,71` and `platforms/karaf/itests/pom.xml:41,47` must be updated first. Reaches **68 of 74** after paying it; the other 6 need bnd wiring from scratch | No packaging change. Instead requires **every jar in the transitive closure** to resolve to a legal module name before `jdeps` will run at all — i.e. a fully ordered install with all ~74 named — plus a central 13-artifact provided-scope exclusion list | `11-bnd:168-219`, `11-bnd:152-166`, `11-bnd:279-296`; `20-mod:229-240`, `20-mod:505-512` |
...
| **Class-file version of the generated `module-info`** | **major 53** (Java 9 format) + `ACC_MODULE` — safe for the Java 11 floor | **Not measured.** Would need `javap -v` on the `module-info.class` extracted from moditect's `kubernetes-model-core` jar (the jdeps intermediate was written under `.../versions/9/module-info.java`, but the emitted class file's version was never checked) | `10-bnd:9-17`; `20-mod:517` |
...
| **Silently omitted `requires`** | **The failure mode is worse than omission: silently WRONG names.** bnd never fails to find *a* name — the brief's target debug string `Can't find a module name for imported package` never appears — it falls back to the raw Maven artifactId. 19 dependency names guessed on one module, none of them legal JPMS names. Two hand-written entries were also omitted outright: `openshift-model-config` (justified — zero bytecode references) and `requires static builder.annotations` (dropped, not marked `static`) | **Names correct, but `transitive` silently lost** on every `requires` — jdeps does not infer re-export. `requires static builder.annotations` also dropped. When a name cannot be resolved, moditect does not emit a wrong one: the build fails outright | `10-bnd:212-292`, `10-bnd:159-195`; `20-mod:537-553`, and the fidelity diffs above |
...
| **Upstream maintenance status** | `maven-bundle-plugin` 5.1.9 / bndlib 6.3.1 pinned (`pom.xml:150`). Newer releases exist: bndlib 6.4.0 (still Java 8+), and `maven-bundle-plugin` 6.0.2 carrying bndlib 7.0.0 — the latter **requires JDK 17 and so is blocked by the JDK 11 CI floor**. The 6.3.1 NPE is unfixed in the pinned version | `moditect-maven-plugin` 1.3.0.Final; upstream recorded as dormant ~13 months | Versions: `pom.xml:150`, `20-mod:17` (plugin version in build output). **Release cadence and dormancy were not measured by any spike step** — the newer-version and dormancy facts are carried from the controller's ledger, not re-derived here. Step that would take it: query Maven Central / the projects' release feeds |
...
`uses` is unreachable, multi-release-jar `requires` names are wrong. If the pin fails, bnd still
wins on cost but needs per-dependency `substitute=` names.

**If it is not acceptable:** bnd is capped at 59 of 74 and misses all 9 artifacts consumers put on
a module path. Then use moditect for those 9 — first confirming it writes a `packaging=jar` main
jar at all, which this spike never did — accepting ~74 name properties, the exclusion list and an
explicit uberjar skip.
...
|---|---|
| moditect writes a descriptor into a `packaging=jar` **main** jar (its headline advantage over bnd) | Install `kubernetes-client-api` with a descriptor, rebuild `httpclient-jdk` (`packaging=jar`), confirm the main jar — not a classified one — carries `module-info.class` |
...
| moditect `module-info.class` class-file version | `javap -v` the `module-info.class` extracted from moditect's `kubernetes-model-core` jar |
...
| Plugin release cadence / moditect dormancy | Query Maven Central and the projects' release feeds |
